#include <math.h>
#include <stdio.h>
#include <stdlib.h>

#define NormRANu (2.3283063671E-10F)

unsigned int irr[256];
unsigned int ir1;
unsigned char ind_ran, ig1, ig2, ig3;

void init_rand(int seed) {
    int ini, factor, sum, i;
    srand(seed);
    ini = seed;
    factor = 67397;
    sum = 7364893;
    for (i = 0; i < 256; i++) {
        ini = (ini * factor + sum);
        irr[i] = ini;
    }
    ind_ran = ig1 = ig2 = ig3 = 0;
}

float Random(void) {
    float r;
    ig1 = ind_ran - 24;
    ig2 = ind_ran - 55;
    ig3 = ind_ran - 61;
    irr[ind_ran] = irr[ig1] + irr[ig2];
    ir1 = (irr[ind_ran] ^ irr[ig3]);
    ind_ran++;
    r = ir1 * NormRANu;
    return r;
}

double my_rand(double a, double b) {
    return a + (double)Random() * (b - a);
}

float target_function(double x) {
    return x * x;
}

void compute_histogram(double data[], double hist[], int n_data, int n_intervals) {
    double max, min, delta;
    int i, valor;
    max = data[0];
    min = data[0];
    
    for (i = 0; i < n_intervals; i++) hist[i] = 0.0;
    
    for (i = 0; i < n_data; i++) {
        if (data[i] > max) max = data[i];
        else if (data[i] < min) min = data[i];
    }

    delta = (max - min) / n_intervals;

    for (i = 0; i < n_data; i++) {
        valor = (int)((data[i] - min) / delta);
        if (valor == n_intervals) valor = n_intervals - 1;
        hist[valor]++;
    }

    for (i = 0; i < n_intervals; i++) {
        hist[i] = hist[i] / n_data;
    }
}

void rejection_sampling(double a, double b, double m_val, double x_arr[], int n_data) {
    int i;
    double x0, y;
    for (i = 0; i < n_data; i++) {
        do {
            x0 = my_rand(a, b);
            y = my_rand(0, m_val);
        } while (y > target_function(x0));
        x_arr[i] = x0;
    }
}

int main() {
    init_rand(12345);
    int n_data = 1000000, i, n_int = 100;
    double a = 0, b = 5, m_val = 25, p;
    double data[n_data], h[n_int];

    rejection_sampling(a, b, m_val, data, n_data);
    compute_histogram(data, h, n_data, n_int);

    FILE *f;
    f = fopen("rejection_sampling.csv", "w");
    fprintf(f, "probability\n");

    for (i = 0; i < n_int; i++) {
        p = h[i];
        fprintf(f, "%lf\n", p);
    }
    fclose(f);

    return 0;
}